...
function GetDesire()
	...
	if npcBot:GetActiveMode() ~= BOT_MODE_ATTACKING and
		...
		if meepoStatus.GetIsFarmed() == true then
			print("farmed +.5")
			desire = desire + .5
		end

		local laneHeadUs = GetLaneFrontAmount( GetTeam(), lane, false )
		local laneHeadThem = GetLaneFrontAmount( utils.GetEnemyTeam(), lane, false )
		desire = 1 - (laneHeadUs + laneHeadThem)
		print(lane .. ":" .. desire)
		return RemapValClamped( desire, BOT_MODE_DESIRE_LOW, BOT_MODE_DESIRE_ABSOLUTE, BOT_MODE_DESIRE_LOW, BOT_MODE_DESIRE_HIGH )
	end
